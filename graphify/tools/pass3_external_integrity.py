...
def main() -> int:
    ...
    if not BASELINE.exists():
        ...
        print(json.dumps({"mode": "pass3-baseline", "file_count": len(files), "byte_count": baseline["byte_count"]}, indent=2))
        return 0
...
""")
    ...
    print(json.dumps({"mode": "pass3-verify", "status": status, "baselineFileCount": baseline["file_count"], "finalFileCount": len(files), "added": len(added), "removed": len(removed), "modified": len(modified), "renamed": len(renamed)}, indent=2))
