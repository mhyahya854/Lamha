"""Pass 1 external read-only integrity baseline and final comparison.

Snapshots every Lamha file outside Graphify except the root ``.git`` metadata
directory, which is Git bookkeeping rather than project content.  All outputs
are written inside Graphify through the write guard.
"""
...
BASELINE = REPORTS / "pass1-external-baseline.json"
FINAL = REPORTS / "pass1-external-final.json"
REPORT = REPORTS / "pass1-external-integrity-report.md"
...
def main() -> int:
    ...
    if not BASELINE.exists():
        ...
        print(json.dumps({"mode": "baseline", "file_count": len(files), "byte_count": baseline["byte_count"]}, indent=2))
        return 0
    ...
    report = f"""# Pass 1 external read-only integrity report
...
Scope excludes only the root `.git` metadata directory (Git bookkeeping).  Zero
differences proves this Pass 1 planning run did not create a backup or alter
project files outside Graphify.
"""
    write_text(REPORT, report)
    print(json.dumps({
        "mode": "verify", "status": status, "baselineFileCount": baseline["file_count"],
